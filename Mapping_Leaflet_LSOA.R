...
pal_fac<-colorFactor(palette = "Purples", levels = 10:1, domain = lsoa_data_nw$`Index.of.Multiple.Deprivation.(IMD).Decile`)
...
leaflet(lsoa_data_nw)%>%
  ...
  addLegend(values = lsoa_data_nw$`Index.of.Multiple.Deprivation.(IMD).Decile`, pal = pal_fac, title = "Attainment Score",position = "topright")%>%
