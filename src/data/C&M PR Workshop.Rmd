---
title: "Cheshire & Merseyside PR Workshop - Analysis Pack"
...
## Create palette
pal_con_a60<-colorNumeric("Oranges", lsoa_data_nw$Pop_60)
...
pop_cm_p<-pop_cm_t%>%
  ...
  summarise(Pop = sum(Pop, na.rm =TRUE))%>%
  mutate(LA = `LA.name.(2021.boundaries)`)%>%
  mutate(Pop2 = case_when(Sex == "Male" ~ -Pop,
                          TRUE ~ Pop))
...
##pop_cm_p%>%
  ##plot_ly(x= ~Pop, y = ~AgeGroup, color = ~Sex)%>%
  ##add_bars(orientation = 'h', hoverinfo = 'text', text = ~Pop)


button_list_master <- list(
  list(
    method = "restyle",
    args = list("transforms[0].value", "Cheshire East"),
    label = "Cheshire East"
  ),
  list(
    method = "restyle",
    args = list("transforms[0].value", "Cheshire West and Chester"), 
    label = "Cheshire West and Chester"
  ),
  list(
    method = "restyle",
    args = list("transforms[0].value", "Halton"), 
    label = "Halton"
  ),
  list(
    method = "restyle",
    args = list("transforms[0].value", "Knowsley"), 
    label = "Knowsley"
  ),
  list(
    method = "restyle",
    args = list("transforms[0].value", "Liverpool"), 
    label = "Liverpool"
  ),
  list(
    method = "restyle",
    args = list("transforms[0].value", "Sefton"), 
    label = "Sefton"
  ),
    list(
    method = "restyle",
    args = list("transforms[0].value", "St. Helens"), 
    label = "St. Helens"
  ),
    list(
    method = "restyle",
    args = list("transforms[0].value", "Warrington"), 
    label = "Warrington"
  ),
    list(
    method = "restyle",
    args = list("transforms[0].value", "Wirral"), 
    label = "Wirral"
  ))

pop_cm_p%>% 
  plot_ly(x = ~`Pop2`, y = ~`AgeGroup`, customdata = pop_cm_p$LA, color = ~Sex, 
          type = 'bar', 
          hoverinfo = 'text', text = ~paste(`AgeGroup`, '</br></br>', 
                                            scales::comma(`Pop`, accuracy=1)),
          transforms = list(
            list(
              type = "filter", 
              target = 'customdata', ##Set the target variable in an attribute above 
              value = ~`LA`))) %>% 
  layout(updatemenus = list(
    list(type = 'dropdown',
         active = 0, 
         buttons = button_list_master,
         x = 0.2,
         y = 1.4,
         xanchor = "center")),
    title = list(text= "Population Pyramid", y = 0.9),
    xaxis = list(title=list(text='Population')),
    yaxis = list(title = list(text= "Age Group", font = list(size = 12), y = 1.2), 
                 tickformat = ",d"), 
  ...
  config(modeBarButtonsToRemove = c("zoomIn2d", "zoomOut2d", "select2d", "lasso2d", "toImage",
                                    "pan2d", "autoScale2d", "resetScale2d", "zoom2d", "hoverClosestCartesian"))
...
## Percentage of Population belonging to an Ethnic Minority {.tabset .tabset-fade}

The map below shows the percentage of residents of each LSOA across Cheshire & Merseyside who identified as belonging to an Ethnic minority group via the 2021 Census. The darker the colour, the higher the percentage identifying as an Ethnic minority in that area.
...
eth<-read.csv("TS021-2021-1-filtered-2022-12-23T11_00_31Z.csv")

eth<-eth%>%
  left_join(lsoa_lk, by = c("Lower.Layer.Super.Output.Areas.Code" = "LSOA.code.(2011)"))

eth<-eth%>%
  left_join(NW_LTLA, by = c("Local.Authority.District.code.(2019)" = "areaCode"))

eth<-eth%>%
  filter(Region == "North West")%>%
  select(c(-4))%>%
  spread(key = 3, value = Observation)

eth<-eth%>%
  mutate("Pop" = rowSums(.[11:30]))%>%
  mutate("Eth_M" = round((1-(`13`/Pop))*100,2))

## Joining LSOA Shape Data and Ethnicity Data
lsoa_data_nw<-merge(lsoa, eth, by.x = "LSOA11CD", "Lower.Layer.Super.Output.Areas.Code")
lsoa_data_nw<-subset(lsoa_data_nw, Region == "North West")
lsoa_data_cm<-subset(lsoa_data_nw, System == "Cheshire & Merseyside")
## Creating Palettes
pal_con<-colorNumeric("Greens", lsoa_data_cm$Eth_M)
...
leaflet(lsoa_data_cm)%>%
  addPolygons(data = lads_cm, color = "black", weight = 2)%>%
  addPolygons(stroke = TRUE, weight = 0.4, opacity = 1, fillOpacity = 0.6,
              color = "grey", fillColor = ~pal_con(lsoa_data_cm$Eth_M),
              label = paste0(lsoa_data_cm$LSOA11NM,": ",lsoa_data_cm$Eth_M,"%"))%>%
  addProviderTiles(providers$CartoDB.Voyager)%>%
  addLegend(values = lsoa_data_cm$Eth_M, pal = pal_con, title = "Population belonging to an Ethnic Minority",position = "topright")%>%
  setView(lng, lat, zoom = 9)
...
The data to produce this map was sourced from the results of the 2021 Census made available by the **Office for National Statistics**. The data can be accessed [here](https://www.ons.gov.uk/datasets/TS021/editions/2021/versions/1/filter-outputs/643a67b0-8b68-4ff5-9ba0-32dc3219d84a#get-data).
...
## Gap in Employment Rate {.tabset .tabset-fade}
...
## Creating Palettes
pal_con<-colorNumeric("Reds", lads_data_cm$Value)
...
## Car and Van Ownership - Census 2021 {.tabset .tabset-fade}
...
## Condition Prevalence {.tabset .tabset-fade}
The maps below show the prevalence of different conditions as recorded on GP systems for each CCG across Cheshire & Merseyside. The darker the colour the higher the prevalence of the condition across that CCG footprint.
...
pal_con<-colorNumeric("Blues", ccg_data_cm$`Prevalence`)
...
leaflet(ccg_data_cm)%>%
  addPolygons(stroke = TRUE, weight = 1, opacity = 1, fillOpacity = 0.6,
              ...
              label = paste0(ccg_data_cm$CCG21NM,": ", round(ccg_data_cm$Prevalence, 2)))%>%
  addProviderTiles(providers$CartoDB.Voyager)%>%
  addLegend(values = ccg_data_cm$Prevalence, pal = pal_con, title = "Prevalence of Obesity (%)",position = "topright")%>%
  setView(lng, lat, zoom = 9)
...
The data to produce these maps was sourced from QOF 2021/22 publication by **NHS Digital**. The data can be accessed [here](https://digital.nhs.uk/data-and-information/publications/statistical/quality-and-outcomes-framework-achievement-prevalence-and-exceptions-data/2021-22).
...
## Create palette
pal_con_asmk<-colorNumeric("Reds", ccg_data_nw$Value)
